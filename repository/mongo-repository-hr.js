...
const {turkishIdentityNoValidator, ibanValidator} = require("../validations");
...
const EmployeeSchema = new Schema({
    "_id": {
        type: String,
        required: true,
        validate: [turkishIdentityNoValidator, "You must provide a valid ID"]
    },
    "fullname": {
        type: String,
        required: true,
...
const getEmployeeByIdentity = (identity) => {
    return EmployeeModel.findOne({"_id": identity}, {});
};

const getEmployees = (pageNo = 0, pageSize = 25) => {
    const skip = pageNo * pageSize;
    const limit = pageSize;
    return EmployeeModel.find({}, {"photo": false},
        {skip, limit});
};
...
exports.getEmployeeByIdentity = getEmployeeByIdentity;
